...
const status = ref('Processing your Twitter connection...')
...
onMounted(async () => {
  ...
  debugLog('[TwitterCallback] Return URL from localStorage:', returnUrl)
  ...
  try {
    ...
    if (error) {
      isError.value = true
      status.value = `Twitter error: ${errorDescription || error}`
    ...
    // Complete OAuth flow
    await twitterStore.handleOAuthCallback(code, state)

    // Success! Clear return URL and redirect
    status.value = `Success! Connected ${twitterStore.connectedAccounts.length} Twitter account(s)`
    localStorage.removeItem('oauth_return_url')
    setTimeout(() => router.push(returnUrl || '/connect-accounts?connected=true'), 2000)
  } catch (error: any) {
    isError.value = true
    status.value = error.message || 'Failed to complete Twitter connection'
    localStorage.removeItem('oauth_return_url')
    setTimeout(() => router.push(returnUrl || defaultRedirect), 3000)
  }
})
    ...
    <div class="container">
      <BaseCard variant="glass" class="callback-card">
        <div class="callback-content">
          ...
          <!-- Additional info -->
          <p v-if="!isError" class="subtitle">Please wait while we connect your Twitter account...</p>
