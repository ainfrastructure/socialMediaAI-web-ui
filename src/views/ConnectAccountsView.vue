...
const pendingDisconnect = ref<{ type: 'facebook' | 'instagram' | 'wolt'; id: string; name: string } | null>(null)
...
onMounted(async () => {
  ...
  if (urlParams.has('connected')) {
    const totalAccounts = facebookStore.connectedPages.length + instagramStore.connectedAccounts.length
    if (totalAccounts > 0) {
...
<template>
  <div class="connect-accounts-view">
    <div class="container">
      ...
      <!-- Error Alerts -->
      <BaseAlert v-if="showErrorMessage && facebookStore.error" type="error" @close="showErrorMessage = false">
        {{ facebookStore.error || $t('connectAccounts.errorOccurred') }}
      </BaseAlert>
      <BaseAlert v-if="showErrorMessage && instagramStore.error" type="error" @close="showErrorMessage = false">
        ...
        <BaseCard variant="glass" class="platforms-list">
          ...
          <div class="platform-row" :class="{ connected: isConnected }">
            ...
            <div class="platform-actions">
              <BaseButton
                v-if="isConnected"
                @click="requestDisconnectPage(facebookStore.connectedPages[0]?.pageId, facebookStore.connectedPages[0]?.pageName)"
                variant="danger"
                size="small"
              ...
              <BaseButton
                ...
                :disabled="facebookStore.loading"
              >
                {{ facebookStore.loading ? $t('connectAccounts.connecting') : $t('connectAccounts.connectToFacebook') }}
              </BaseButton>
            </div>
          </div>
          ...
          <div class="platform-row" :class="{ connected: isInstagramConnected }">
            <div class="platform-icon" style="background: linear-gradient(135deg, #F58529, #DD2A7B, #8134AF, #515BD4);">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="white" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="platform-actions">
              <BaseButton
                ...
                @click="requestDisconnectInstagramAccount(instagramStore.connectedAccounts[0]?.instagramAccountId, instagramStore.connectedAccounts[0]?.username)"
                variant="danger"
                size="small"
          ...
          <div class="platform-row" :class="{ connected: isWoltConnected }">
            ...
            <div class="connected-account" v-if="isWoltConnected">
              <span class="account-name">Connected</span>
              <span class="connected-badge">
                <svg class="checkmark-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M9 12L11 14L15 10" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
