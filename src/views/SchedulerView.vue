<template>
  <DashboardLayout>
    ...
    <div class="container">
      ...
      <BaseCard variant="glass" class="calendar-card">
        ...
        <CalendarFilters
          ...
          v-model:selectedBusinesses="filters.business_ids"
          @apply="applyFilters"
        />
            ...
            <div v-if="day.posts.length > 0" class="day-posts">
              ...
              <div v-if="viewMode === 'day'" class="day-view-table">
                ...
                <div class="table-header">
                  ...
                  <div class="th-restaurant">{{ $t('scheduler.restaurant') || 'Business' }}</div>
    ...
    <BusinessSelectorModal
      ...
      :current-id="preferencesStore.selectedBusinessId || undefined"
...
const fetchScheduledPosts = async () => {
  try {
    ...
    scheduledPosts.value = uniquePosts.map((post: any) => {
        ...
        let businessName = post.business_name || post.businesses?.name || post.business?.name
        ...
        if (!mediaUrl && post.favorite_posts) {
          ...
          businessName = businessName || post.favorite_posts.businesses?.name || post.favorite_posts.business_name
        ...
        if (!mediaUrl && post.favorite) {
          ...
          businessName = businessName || post.favorite.businesses?.name || post.favorite.business_name
        ...
        if (!mediaUrl && post.favorite_post) {
          ...
          businessName = businessName || post.favorite_post.businesses?.name || post.favorite_post.business_name
...
const selectCreationMethod = (method: 'saved' | 'new') => {
  ...
  if (method === 'new') {
    const preferredBusinessId = filters.value.business_ids.length === 1
      ? filters.value.business_ids[0]
      : (preferencesStore.selectedBusinessId || undefined)

    if (preferredBusinessId) {
      proceedWithCreationMethod(method, preferredBusinessId)
    } else if (businesses.value.length === 1) {
      proceedWithCreationMethod(method, businesses.value[0].id)
    } else {
  ...
  // Saved post flow uses business selection
  if (filters.value.business_ids.length === 1) {
    selectedBusinessIdForPosts.value = filters.value.business_ids[0]
  } else {
    selectedBusinessIdForPosts.value = preferencesStore.selectedBusinessId || undefined
  }
...
const proceedWithCreationMethod = (method: 'saved' | 'new', businessId?: string) => {
  if (method === 'new') {
    ...
    if (businessId) {
      preferencesStore.setSelectedBusiness(businessId)
    }
    router.push(url)
  }
}
