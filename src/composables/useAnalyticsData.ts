...
export function useAnalyticsData() {
  ...
  const selectedBusiness = computed(() =>
    businessesStore.businesses.find(b => b.id === selectedBusinessId.value) || null
  )
  ...
  const businessFilteredScheduledPosts = computed(() => {
    if (!selectedBusinessId.value) return scheduledPosts.value
    return scheduledPosts.value.filter(post =>
      post.business_id === selectedBusinessId.value ||
      post.businesses?.id === selectedBusinessId.value ||
      post.business?.id === selectedBusinessId.value ||
      post.restaurant_id === selectedBusinessId.value ||
      post.favorite_posts?.business_id === selectedBusinessId.value ||
      post.favorite_post?.business_id === selectedBusinessId.value ||
      post.favorite?.business_id === selectedBusinessId.value
    )
  })

  const businessFilteredFavorites = computed(() => {
    if (!selectedBusinessId.value) return favorites.value
    return favorites.value.filter(fav =>
      fav.business_id === selectedBusinessId.value ||
      fav.businesses?.id === selectedBusinessId.value ||
      fav.business?.id === selectedBusinessId.value ||
      fav.restaurant_id === selectedBusinessId.value
    )
  })

  const timeRangeFilteredPosts = computed(() => {
    if (selectedTimeRange.value === 'all') return businessFilteredScheduledPosts.value
    return businessFilteredScheduledPosts.value.filter(post => {
  ...
  const filteredFavorites = computed(() => {
    if (selectedTimeRange.value === 'all') return businessFilteredFavorites.value
    return businessFilteredFavorites.value.filter(fav => {
  ...
  const trendData = computed(() => {
    ...
    const allPosts = businessFilteredScheduledPosts.value
    ...
    const currentFavs = businessFilteredFavorites.value.filter(f => {
      const d = f.created_at || f.saved_at
      if (!d) return false
      const t = new Date(d).getTime()
      return t >= currentStart && t < now
    })
    const prevFavs = businessFilteredFavorites.value.filter(f => {
      const d = f.created_at || f.saved_at
      if (!d) return false
      const t = new Date(d).getTime()
  ...
  const hasMeaningfulStats = computed(() =>
    businessFilteredScheduledPosts.value.length > 0 || businessFilteredFavorites.value.length > 0
  )
  ...
  function getBusinessName(post: any): string {
    ...
    const businessId = post.business_id || post.favorite_posts?.business_id ||
      post.favorite_post?.business_id || post.favorite?.business_id
    if (businessId && businessesStore.businesses.length > 0) {
      const business = businessesStore.businesses.find((b: any) => b.id === businessId)
  ...
  async function fetchAnalyticsData() {
    ...
    try {
      ...
      const published = businessFilteredScheduledPosts.value.filter(p => p.status === 'published')
  ...
  return {
    ...
    businessFilteredScheduledPosts,
    businessFilteredFavorites,
