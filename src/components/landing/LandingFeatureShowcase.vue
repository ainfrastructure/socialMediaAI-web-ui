...
const features: Feature[] = [
  {
    id: 'analytics',
    icon: 'insights',
    tabKey: 'appLanding.featureShowcase.tabAnalytics',
    eyebrowKey: 'appLanding.analytics.eyebrow',
    titleKey: 'appLanding.analytics.title',
    subtitleKey: 'appLanding.analytics.subtitle',
  },
  {
    id: 'calendar',
    icon: 'calendar_month',
    tabKey: 'appLanding.featureShowcase.tabCalendar',
    eyebrowKey: 'appLanding.calendar.eyebrow',
    titleKey: 'appLanding.calendar.title',
    subtitleKey: 'appLanding.calendar.subtitle',
  },
  {
    id: 'competitor',
    icon: 'radar',
    tabKey: 'appLanding.featureShowcase.tabCompetitor',
    eyebrowKey: 'appLanding.competitor.eyebrow',
    titleKey: 'appLanding.competitor.title',
    subtitleKey: 'appLanding.competitor.subtitle',
  },
  {
    id: 'designBuilder',
    icon: 'palette',
    tabKey: 'appLanding.featureShowcase.tabDesignBuilder',
    eyebrowKey: 'appLanding.designBuilder.eyebrow',
    titleKey: 'appLanding.designBuilder.title',
    subtitleKey: 'appLanding.designBuilder.subtitle',
  },
  {
    id: 'ads',
    icon: 'campaign',
    tabKey: 'appLanding.featureShowcase.tabAds',
    eyebrowKey: 'appLanding.adsManager.eyebrow',
    titleKey: 'appLanding.adsManager.title',
...
// Schedule highlights for calendar sub-content
const scheduleItems = [
...
async function switchTo(index: number, userInitiated = true) {
  ...
  await Promise.all([
    gsap.to(textPanelRef.value, {
      opacity: 0, x: exitDir * 40,
      duration: 0.3, ease: 'power2.in',
    }),
    gsap.to(mockupPanelRef.value, {
      opacity: 0, x: exitDir * 80, scale: 0.95,
      duration: 0.3, ease: 'power2.in',
    }),
  ])
  ...
  if (mockupPanelRef.value) {
    gsap.fromTo(mockupPanelRef.value,
      { opacity: 0, x: enterDir * 80, scale: 0.95 },
      {
...
function handleKeydown(e: KeyboardEvent) {
  if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
    e.preventDefault()
    switchTo((activeIndex.value + 1) % features.length)
  } else if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
    e.preventDefault()
    switchTo((activeIndex.value - 1 + features.length) % features.length)
  }
}
        ...
        <div ref="textPanelRef" class="lp-showcase-text">
          ...
          <div v-if="activeFeature.id === 'analytics'" class="lp-kpi-grid">
            <div v-for="kpi in kpis" :key="kpi.key" class="lp-kpi-card">
              <div class="lp-kpi-icon">
                <MaterialIcon :icon="kpi.icon" size="sm" />
              </div>
          ...
          <div v-if="activeFeature.id === 'calendar'" class="lp-layers">
            ...
            >
              ...
              <div class="lp-layer-info">
                <span class="lp-layer-name">{{ t(`appLanding.calendar.${item.key}`) }}</span>
                <span class="lp-layer-desc">{{ t(`appLanding.calendar.${item.key}Desc`) }}</span>
              </div>
          ...
          <div v-if="activeFeature.id === 'ads'" class="lp-kpi-grid">
            <div v-for="stat in adsStats" :key="stat.key" class="lp-kpi-card">
              <div class="lp-kpi-icon">
                <MaterialIcon :icon="stat.icon" size="sm" />
              </div>
              <div class="lp-kpi-content">
...
.lp-layer-item:hover {
  border-color: var(--layer-color, var(--lp-border-light));
}
...
@media (max-width: 768px) {
  ...
  .lp-showcase-tabs::-webkit-scrollbar {
    display: none;
  }
