...
/**
 * Variant B — "Directional Slide"
 * Content slides out in travel direction, new content slides in from opposite side
 * Strong horizontal parallax between text and mockup panels
 */
...
async function switchTo(index: number, userInitiated = true) {
  ...
  // ── EXIT: Slide out in travel direction (text fast, mockup slower = parallax) ──
  await Promise.all([
    gsap.to(textPanelRef.value, {
      opacity: 0, x: exitDir * 120,
      duration: 0.4, ease: 'power3.in',
    }),
    gsap.to(mockupPanelRef.value, {
      opacity: 0, x: exitDir * 200,
...
.lp-showcase-tabs {
  ...
  margin-bottom: var(--space-4xl); flex-wrap: wrap;
}

.lp-showcase-tab {
  display: flex; align-items: center; gap: var(--space-sm);
  padding: var(--space-md) var(--space-xl);
  border: 1px solid var(--lp-border); border-radius: var(--radius-full);
  background: var(--lp-bg-surface); color: var(--lp-text-secondary);
  font-size: var(--text-sm); font-weight: var(--font-semibold);
  cursor: pointer; transition: all 0.3s ease;
  position: relative;
}
.lp-showcase-tab:hover { border-color: var(--lp-border-light); color: var(--lp-text-primary); }
.lp-showcase-tab.active { border-color: var(--lp-accent-orange); color: var(--lp-accent-orange); background: rgba(249, 115, 22, 0.06); }
