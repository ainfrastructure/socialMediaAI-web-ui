...
const emit = defineEmits<{
  ...
  (e: 'update:selectedBusinessId', id: string | null): void
  (e: 'refresh'): void
}>()
...
function handleBusinessSelected(business: { id: string }) {
  emit('update:selectedBusinessId', business.id)
}
  ...
  <div class="analytics-header">
    ...
    <div class="hero-banner">
      ...
      <div class="hero-right">
        ...
        <button v-if="businesses.length > 0" class="business-chip" @click="showBusinessSelector = true">
          <div v-if="selectedBusiness?.logo_url || selectedBusiness?.brand_dna?.logo_url" class="chip-avatar">
          ...
          <span class="chip-name">{{ selectedBusiness?.name || t('analytics.selectBusiness') }}</span>
          <MaterialIcon icon="swap_horiz" size="xs" class="chip-action-icon" />
        </button>
        <button v-else class="add-business-btn" @click="router.push('/businesses')">
...
.business-chip:hover .chip-action-icon {
  color: var(--gold-primary);
}
